...
function App() {
  ...
  const [records, setrecords] = useState(() => {
    let init = localStorage.getItem("records");
  ...
  async function decryptPassword(
    ...
    try {
      ...
      toast.info("Decryption failed! Possible wrong key or corrupted data", {
        position: "top-right",
        autoClose: 1000,
  ...
  useEffect(() => {
    ...
    let savedRecords = JSON.parse(saved);
    setrecords(savedRecords);
  }, []);

  useEffect(() => {
    // guard if records is null / not an array
    const safeRecords = Array.isArray(records) ? records : [];

    // create sanitized copy — does not mutate original objects
    const sanitized = safeRecords.map((obj) => {
      // Use default {} to avoid destructuring undefined
